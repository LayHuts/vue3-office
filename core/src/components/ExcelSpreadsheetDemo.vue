<script setup>
import { ref, onMounted, nextTick } from 'vue';
import zhCN from 'x-data-spreadsheet/src/locale/zh-cn';
import Spreadsheet from 'x-data-spreadsheet';

import 'x-data-spreadsheet/dist/xspreadsheet.css';
import '../assets/xspreadsheet-official.css';

const rootRef = ref(null);
const wrapperRef = ref(null);
let xs = null;


onMounted(async () => {
  nextTick(() => {
    // 生成大量数据用于测试（官方 demo 数据）
    const rows10 = { len: 1000 };
    for (let i = 0; i < 1000; i += 1) {
      rows10[i] = {
        cells: {
          0: { text: 'A-' + i },
          1: { text: 'B-' + i },
          2: { text: 'C-' + i },
          3: { text: 'D-' + i },
          4: { text: 'E-' + i },
          5: { text: 'F-' + i },
        }
      };
    }

    // 官方 demo 的示例数据
    const rows = {
      len: 80,
      1: {
        cells: {
          0: { text: 'testingtesttestetst' },
          2: { text: 'testing' },
        },
      },
      2: {
        cells: {
          0: { text: 'render', style: 0 },
          1: { text: 'Hello' },
          2: { text: 'haha', merge: [1, 1] },
        }
      },
      8: {
        cells: {
          8: { text: 'border test', style: 0 },
        }
      }
    };

    // SVG 图标（Base64 编码）
    const saveIcon = 'data:image/svg+xml;base64,';

    // 创建预览图标元素
    const previewEl = document.createElement('img');
    previewEl.src = 'data:image/svg+xml;base64,';
    previewEl.width = 16;
    previewEl.height = 16;

    // 初始化 x-data-spreadsheet（使用官方 demo 配置 + 扩展工具栏）
    console.log('初始化 Spreadsheet，saveIcon:', saveIcon);
    console.log('previewEl:', previewEl);
    Spreadsheet.locale('zh-cn', zhCN);
    xs = new Spreadsheet(rootRef.value, {
      showToolbar: true,
      showGrid: true,
      showBottomBar: true,
      view: {
        height: () => wrapperRef.value?.clientHeight || 600,
        width: () => wrapperRef.value?.clientWidth || 1200,
      },
      extendToolbar: {
        left: [
          {
            tip: '保存',
            icon: saveIcon,
            onClick: (data, sheet) => {
              console.log('点击保存按钮:', data, sheet);
              exportData();
            }
          }
        ],
        right: [
          {
            tip: '预览',
            el: previewEl,
            onClick: (data, sheet) => {
              console.log('点击预览按钮:', data);
              alert('预览功能：数据已输出到控制台');
            }
          }
        ],
      }
    });

    console.log('Spreadsheet 初始化完成:', xs);

    // 加载官方 demo 数据
    xs.loadData([
      {
        freeze: 'B3',
        styles: [
          {
            bgcolor: '#f4f5f8',
            textwrap: true,
            color: '#900b09',
            border: {
              top: ['thin', '#0366d6'],
              bottom: ['thin', '#0366d6'],
              right: ['thin', '#0366d6'],
              left: ['thin', '#0366d6'],
            },
          },
        ],
        merges: ['C3:D4'],
        cols: {
          len: 10,
          2: { width: 200 },
        },
        rows,
      },
      { name: 'sheet-test', rows: rows10 }
    ]).change((cdata) => {
      console.log('数据变化:', xs.getData());
    });

    // 隐藏初始选中状态（避免默认选中 [0,0] 单元格）
      if (xs && xs.sheet && xs.sheet.selector) {
        // xs.sheet.data.selector.range = { sri: -1, sci: -1, eri: -1, eci: -1 };
        xs.sheet.selector.hide();
      }

    // 监听单元格选中事件
    xs.on('cell-selected', (cell, ri, ci) => {
      console.log('选中单元格 - cell:', cell, ', ri:', ri, ', ci:', ci);
    });

    // 监听单元格编辑事件
    xs.on('cell-edited', (text, ri, ci) => {
      console.log('编辑单元格 - text:', text, ', ri:', ri, ', ci:', ci);
    });

    // 测试：5秒后修改单元格内容
    setTimeout(() => {
      xs.cellText(14, 3, 'cell-text').reRender();
      console.log('cell(8, 8):', xs.cell(8, 8));
      console.log('cellStyle(8, 8):', xs.cellStyle(8, 8));
    }, 5000);
  });
});

// 导出数据
function exportData() {
  if (xs) {
    const data = xs.getData();
    console.log('导出数据:', data);
    alert('数据已输出到控制台');
  }
}

// 清空数据
function clearData() {
  if (xs) {
    xs.loadData({});
  }
}

// 重新加载示例数据
function reloadData() {
  if (xs) {
    xs.loadData({
      name: 'Sheet1',
      rows: {
        0: {
          cells: {
            0: { text: 'Hello' },
            1: { text: 'World' },
          },
        },
      },
    });
  }
}
</script>

<template>
  <div class="spreadsheet-demo">
    <div class="demo-header">
      <h2>x-data-spreadsheet 官方示例</h2>
      <div class="demo-actions">
        <button @click="exportData">导出数据</button>
        <button @click="reloadData">重新加载</button>
        <button @click="clearData">清空</button>
      </div>
    </div>
    <div class="spreadsheet-wrapper" ref="wrapperRef">
      <div ref="rootRef" id="x-spreadsheet-demo"></div>
    </div>
  </div>
</template>

<style scoped>
.spreadsheet-demo {
  display: flex;
  flex-direction: column;
  height: calc(100vh - 46px);
  padding: 20px;
  box-sizing: border-box;
  overflow: hidden;
}

.demo-header {
  flex-shrink: 0;
  margin-bottom: 20px;
}

.demo-header h2 {
  margin: 0 0 10px 0;
  color: #333;
}

.demo-actions {
  display: flex;
  gap: 10px;
}

.demo-actions button {
  padding: 8px 16px;
  background: #4b89ff;
  color: white;
  border: none;
  border-radius: 4px;
  cursor: pointer;
  font-size: 14px;
  transition: background 0.3s;
}

.demo-actions button:hover {
  background: #3a78ee;
}

.spreadsheet-wrapper {
  flex: 1;
  min-height: 0;
  border: 1px solid #e0e2e4;
  border-radius: 4px;
  overflow: hidden;
  background: #fff;
}

.spreadsheet-wrapper * {box-sizing: content-box}
</style>